...
pub mod PredictionHub {
    ...
    impl AdditionalAdminImpl of IAdditionalAdmin<ContractState> {
        ...
        fn emergency_close_market(ref self: ContractState, market_id: u256, market_type: u8) {
            ...
            assert(market.is_open, 'Market is already closed');
            assert(!market.is_resolved, 'Market is already resolved');

            // Close the market but don't resolve it
            market.is_open = false;
            market.status = MarketStatus::Locked;

            self.all_predictions.entry(market_id).write(market);

            self.emit(MarketClosed { market_id, closer: get_caller_address() });
        }
        ...
        fn emergency_resolve_market(
            ...
            // Emergency resolve allows resolving before end time and without time checks
            market.is_resolved = true;
            market.is_open = false;
